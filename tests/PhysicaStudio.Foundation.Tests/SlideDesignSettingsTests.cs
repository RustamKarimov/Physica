...
public sealed class SlideDesignSettingsTests
{
    ...
    public void DesktopExposesDesignCommandsThroughContextualInspector()
    {
        ...
        var angleEditorXaml = File.ReadAllText(Path.Combine(root, "src", "PhysicaStudio.Desktop", "Controls", "PhysicaAngleEditor.axaml"));
        var angleEditorCode = File.ReadAllText(Path.Combine(root, "src", "PhysicaStudio.Desktop", "Controls", "PhysicaAngleEditor.axaml.cs"));
        var angleSliderCode = File.ReadAllText(Path.Combine(root, "src", "PhysicaStudio.Desktop", "Controls", "PhysicaAngleSlider.cs"));
        ...
        Assert.Contains("controls:PhysicaAngleEditor", gradientEditorXaml, StringComparison.Ordinal);
        Assert.DoesNotContain("<Slider", gradientEditorXaml, StringComparison.Ordinal);
        ...
        Assert.Contains("ColumnDefinitions=\"*,72\"", angleEditorXaml, StringComparison.Ordinal);
        Assert.Contains("Classes=\"angle-value-input\"", angleEditorXaml, StringComparison.Ordinal);
        Assert.Contains("Classes=\"angle-degree-suffix\"", angleEditorXaml, StringComparison.Ordinal);
        Assert.Contains("AngleTextBox_PointerWheelChanged", angleEditorCode, StringComparison.Ordinal);
        Assert.Contains("Key.PageUp", angleEditorCode, StringComparison.Ordinal);
        Assert.Contains("for (var index = 0; index <= 4; index++)", angleSliderCode, StringComparison.Ordinal);
        Assert.Contains("OnPointerPressed", angleSliderCode, StringComparison.Ordinal);
        Assert.Contains("OnPointerWheelChanged", angleSliderCode, StringComparison.Ordinal);
