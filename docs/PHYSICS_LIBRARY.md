...
Owns drag-and-drop smart models, prefabs, visual assets, instruments, representations and materials.

## Scope

Built-in/Plugin/My Library, canonical IDs, anchors, ports, compatibility, snapping, prefab snapshots and provenance.
...
- instantiate snapshots into projects
- declare anchors/ports and target compatibility
- tag items by curriculum/topic

## This subsystem MUST NOT

- silently update old project instances when library definitions change
- invent physics when a visual-only asset is dropped
...
- drag smart model/prefab/instrument
...
## Text Library presets

The built-in Representation Objects include `Text Block`, `Definition`, `Explanation`, `Caption`, `Callout`, `Quote`, `Bullet List`, `Examiner Note` and `Warning`. These are presets of canonical `TextBlock`, not separate physics or root schema types.
