...
| --- | --- | --- | --- |
| Model/service and render contracts | 91 passed on 2026-09-13. Viewport scenarios cover fit mathematics, pointer anchoring, pan recovery bounds, per-slide restoration/project reset, zoom limits, and constant-screen-size authoring overlays; the prior grouping, document, render, history, naming, section, and snapping suites remain green | Validation, serialization, command results, history, selection stability, transform authority, viewport geometry/session state, hierarchy, layer order, naming, section semantics, snapping calculations, scene mapping, aspect math, and invalidation | Real pointer, keyboard, focus, pixel equivalence, drag/drop, touchpad behavior, or usability |
...
| End-to-end | The user manually accepted the corrected canvas and reviewed Layers behavior, including the final double-click rename correction. The viewport build launched as exactly one responsive Windows process, and a repeated launcher invocation refused a duplicate. Computer initialization failed before observation after reset and retry | Process startup, duplicate-instance protection, and the user's accepted earlier canvas/Layers workflows | Grouping and viewport interaction, controlled visual equivalence, DPI, macOS behavior, and final Phase 2 approval |
