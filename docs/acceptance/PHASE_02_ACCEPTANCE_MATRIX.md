...
| --- | --- | --- | --- |
| Model/service and render contracts | 80 passed on 2026-09-12. Five grouping scenarios add atomic group/ungroup history, nesting/collapse, inherited lock/visibility/transform behavior, contiguous sibling ordering, deletion, serialization, and ungroup render equivalence | Validation, serialization, command results, history, selection stability, transform authority, resize modifier geometry, hierarchy, layer order, automatic/custom naming, section semantics, snapping calculations, scene mapping, aspect math, and invalidation | Real pointer, keyboard, focus, pixel equivalence, drag/drop, or usability |
| UI component | Canvas-render/geometry scenarios pass; structural scenarios verify canvas and Layers event wiring, including group/ungroup controls and group disclosure | Geometry-aware surface behavior and static reachability of real controls | Actual pointer capture, modifier input, grouping usability, complete lifecycle, or cross-platform behavior |
...
| End-to-end | The user manually accepted the corrected canvas and reviewed Layers behavior, including the final double-click rename correction. The grouping build launched as exactly one responsive Windows process, but Computer initialization failed before input after reset and retry | Process startup and the user's accepted canvas/Layers workflows | Grouping interaction, controlled visual equivalence, DPI, macOS behavior, and final Phase 2 approval |
