...
| Requirement | Current state | Blocking finding | Required evidence |
| --- | --- | --- | --- |
| Canvas selection, multi-selection, transforms, layers, grouping, lock, and hide | UI wired | The user accepted the corrected marquee and PowerPoint-style resize modifiers. In the first Layers review, view switching, canvas/list selection, F2 rename, Forward, Front/Back, undo/redo, and deletion passed; the header appearance, double-click rename, direct dragging, and Backward failed. Commit `f14c411` corrects those defects and passes 75 checks, but awaits the user's real-app retest. Grouping and controlled cross-platform evidence remain incomplete | Retest the corrected Layers header, double-click rename, direct drag with insertion feedback, and Forward/Backward inverse; then grouping workflow and one undo entry per gesture |
...
| --- | --- | --- | --- |
| Model/service and render contracts | 75 passed on 2026-09-09. Layer scenarios cover range selection, atomic multi-object reordering, lock/visibility/rename validation, boundary movement, exact one-layer forward/backward inverse, view-model synchronization, and undo | Validation, serialization, command results, history, selection stability, transform authority, resize modifier geometry, layer order, automatic/custom naming, section semantics, snapping calculations, scene mapping, aspect math, and invalidation | Real pointer, keyboard, focus, pixel equivalence, drag/drop, or usability |
...
| End-to-end | The user manually accepted corrected canvas behavior and partially reviewed Layers. That review passed switching, selection, F2, most z-order actions, undo/redo, and deletion, while rejecting the header and finding double-click, drag, and Backward defects. Correction `f14c411` passes 75 checks. Computer initialization failed before input, so the corrected interactions remain unverified in the running application | Process startup, the user's canvas acceptance, and the first Layers review | Retest the corrected header, double-click, direct drag, and one-step Backward; visual equivalence, cross-platform behavior, and final Layers approval |
