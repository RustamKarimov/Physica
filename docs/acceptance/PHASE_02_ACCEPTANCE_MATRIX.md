...
| --- | --- | --- | --- |
| Model/service and render contracts | 103 passed on 2026-09-13. Guidance checks protect ruler modes, exact grid rendering structure, one authoritative Guides workspace, and the single-control dock/float boundary; prior overlay, round-trip, undo, transform, snapping, viewport, grouping, document, render, history, naming, and section suites remain green | Validation, serialization, command results, history, selection stability, transform authority, viewport geometry/session state, hierarchy, layer order, guidance settings, snapping calculations, scene mapping, aspect math, and static workspace architecture | Real pointer, keyboard, focus, actual float/dock interaction, pixel equivalence, drag/drop, touchpad behavior, or usability |
...
| End-to-end | The user manually accepted the corrected canvas and reviewed Layers behavior, including the final double-click rename correction. Commit `ee3239c` launched as exactly one responsive Windows process after its build and 103-check suite passed. Computer initialization failed before application observation after reset and retry | Process startup and the user's accepted earlier canvas/Layers workflows | Dock/float behavior, corrected grid/ruler/guide interaction and usability, grouping, viewport, controlled visual equivalence, DPI, macOS, and final Phase 2 gate approval |
