...
| Themes, slide backgrounds, size, orientation, margins, and safe-area values | UI wired | The first teacher review failed because Slide Design overlapped the Standing Wave inspector, theme variants were not meaningfully distinct, and colours required hexadecimal entry. The current recovery replaces the cramped bespoke colour bars/HSL editor with compact official ColorPicker controls (contextual palette, spectrum, components, and exact-value entry), adds a clear custom-background override notice, and routes the Themes command to the gallery. The solution build passes with zero warnings/errors, the focused suite is 110/110, and the official launcher opens one responsive instance. Computer failed before observation after reset and retry | Teacher review of the recovered inspector, nine theme variants, compact primary/secondary colour flyouts, override action, editor/thumbnail/presenter agreement, both resize policies, grouped content, overlay agreement, undo/redo, and save/reopen |
...
| --- | --- | --- | --- |
| Model/service and render contracts | 110 passed on 2026-09-13. Seven slide-design checks protect resize policies, model-transform authority, nested-group scaling, guide relocation, theme/background/inset round trips, undo, shared background resolution, inspector exclusion, the nine distinct palettes, and structural Design routing; the colour-picker controls and Themes routing are now structurally protected; all prior suites remain green | Validation, serialization, command results, history, selection stability, transform authority, viewport geometry/session state, hierarchy, layer order, guidance/settings, snapping calculations, scene mapping, canvas resizing, theme/background resolution, aspect math, and static workspace architecture | Real pointer, keyboard, focus, actual float/dock interaction, pixel equivalence, colour appearance, drag/drop, touchpad behavior, or usability |
