...
- Angles use one reusable `PhysicaAngleEditor`, not a stock theme slider beside a generic text box. It combines a restrained Physica-accent rail, visible 0°/90°/180°/270°/360° reference ticks, a compact 11 px visual thumb with a larger invisible pointer target, and one integrated numeric field with a separate degree suffix. Dragging and exact typing are equivalent; arrows refine by 1°, Shift+arrow and Page Up/Page Down use 15°, Home selects 0°, End selects 360°, and the pointer wheel adjusts without requiring a different editor. The same control is reserved for later shape rotation, vectors, cameras, paths, and object formatting.
